...
And
 (Forall ("m",
   Forall ("n",
    Imp (Atom (R ("=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
     Atom (R ("=", [Var "m"; Var "n"]))))),
 And
  (Forall ("n",
    Iff (Not (Atom (R ("=", [Var "n"; Fn ("0", [])]))),
     Exists ("m", Atom (R ("=", [Var "n"; Fn ("S", [Var "m"])]))))),
  And
   (Forall ("n",
     Atom (R ("=", [Fn ("+", [Fn ("0", []); Var "n"]); Var "n"]))),
   And
    (Forall ("m",
      Forall ("n",
       Atom
    ...
    And
     ...
     And
      ...
      And
       (Forall ("m",
         Forall ("n",
          Iff (Atom (R ("<=", [Var "m"; Var "n"])),
           Exists ("d",
            Atom (R ("=", [Fn ("+", [Var "m"; Var "d"]); Var "n"])))))),
       Forall ("m",
        Forall ("n",
         Iff (Atom (R ("<", [Var "m"; Var "n"])),
          Atom (R ("<=", [Fn ("S", [Var "m"]); Var "n"]))))))))))))
...
And
 (Forall ("n",
   Imp (Atom (R ("=", [Fn ("S", [Var "n"]); Fn ("0", [])])), False)),
 And
  (Forall ("n",
    Imp (Atom (R ("=", [Fn ("0", []); Fn ("S", [Var "n"])])), False)),
  And
   (Forall ("m",
     Forall ("n",
      Imp (Imp (Atom (R ("=", [Var "m"; Var "n"])), False),
       Imp (Atom (R ("=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
        False)))),
   And
    (Forall ("m",
     ...
     And
      (Forall ("m",
        Forall ("n",
         Imp
          (Forall ("d",
            Imp (Atom (R ("<=", [Var "d"; Var "n"])),
             Imp (Atom (R ("=", [Var "d"; Var "m"])), False))),
          Imp (Atom (R ("<=", [Var "m"; Var "n"])), False)))),
      And
       (Forall ("m",
         Forall ("n",
          Imp
       ...
       And
        (Forall ("n",
          Or (Atom (R ("<=", [Var "n"; Fn ("0", [])])),
           Exists ("m", Atom (R ("=", [Fn ("S", [Var "m"]); Var "n"]))))),
        And
         (Forall ("n",
           Imp (Atom (R ("<=", [Var "n"; Fn ("0", [])])),
            Atom (R ("=", [Var "n"; Fn ("0", [])])))),
         And
          (Forall ("m",
            Forall ("n",
             Imp
              (Atom
                (R ("<=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
              Atom (R ("<=", [Var "m"; Var "n"]))))),
          And
           (Forall ("m",
             Forall ("n",
              Imp (Atom (R ("<", [Var "m"; Fn ("S", [Var "n"])])),
               Atom (R ("<=", [Var "m"; Var "n"]))))),
           Forall ("n",
            Imp (Atom (R ("<", [Var "n"; Fn ("0", [])])), False)))))))))))))

let robinson_thm =
  prove (Imp(robinson,robinson_consequences))
  [note("eq_refl",Forall ("x", Atom (R ("=", [Var "x"; Var "x"])))) using [axiom_eqrefl (Var "x")];
   note("eq_trans",Forall ("x",
 Forall ("y",
  Forall ("z",
   Imp (Atom (R ("=", [Var "x"; Var "y"])),
    Imp (Atom (R ("=", [Var "y"; Var "z"])),
     Atom (R ("=", [Var "x"; Var "z"]))))))))
      using [eq_trans (Var "x") (Var "y") (Var "z")];
   note("eq_sym",Forall ("x",
 Forall ("y", Imp (Atom (R ("=", [Var "x"; Var "y"])), Atom (R ("=", [Var "y"; Var "x"]))))))
      using [eq_sym (Var "x") (Var "y")];
   note("suc_cong",Forall ("a", Forall ("b", Imp (Atom (R ("=", [Var "a"; Var "b"])),
   Atom (R ("=", [Fn ("S", [Var "a"]); Fn ("S", [Var "b"])]))))))
      using [axiom_funcong "S" [Var "a"] [Var "b"]];
   note("add_cong", Forall ("a", Forall ("b", Forall ("c", Forall ("d",
    Imp (And (Atom (R ("=", [Var "a"; Var "b"])),
       Atom (R ("=", [Var "c"; Var "d"]))),
     Atom (R ("=",
        [Fn ("+", [Var "a"; Var "c"]); Fn ("+", [Var "b"; Var "d"])]))))))))
      using [axiom_funcong "+" [Var "a"; Var "c"] [Var "b"; Var "d"]];
   note("le_cong", Forall ("a", Forall ("b", Forall ("c", Forall ("d", Imp
     (And (Atom (R ("=", [Var "a"; Var "b"])),
       Atom (R ("=", [Var "c"; Var "d"]))),
     Imp (Atom (R ("<=", [Var "a"; Var "c"])),
      Atom (R ("<=", [Var "b"; Var "d"])))))))))
      using [axiom_predcong "<=" [Var "a"; Var "c"] [Var "b"; Var "d"]];
   note("lt_cong", Forall ("a", Forall ("b", Forall ("c", Forall ("d", Imp
     (And (Atom (R ("=", [Var "a"; Var "b"])),
       Atom (R ("=", [Var "c"; Var "d"]))),
     Imp (Atom (R ("<", [Var "a"; Var "c"])),
      Atom (R ("<", [Var "b"; Var "d"])))))))))
      using [axiom_predcong "<" [Var "a"; Var "c"] [Var "b"; Var "d"]];
   assume ["suc_inj",Forall ("m",
 Forall ("n",
  Imp (Atom (R ("=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
   Atom (R ("=", [Var "m"; Var "n"])))));
           "num_nz",Forall ("n",
 ...
 Forall ("n",
  Iff (Atom (R ("<", [Var "m"; Var "n"])),
   Atom (R ("<=", [Fn ("S", [Var "m"]); Var "n"])))))];
   note("not_suc_0",(Forall ("n", Not (Atom (R ("=", [Fn ("S", [Var "n"]); Fn ("0", [])])))))) by ["num_nz"; "eq_refl"];
   so conclude (Forall ("n",
 Imp (Atom (R ("=", [Fn ("S", [Var "n"]); Fn ("0", [])])), False))) at once;
   so conclude (Forall ("n",
 Imp (Atom (R ("=", [Fn ("0", []); Fn ("S", [Var "n"])])), False))) by ["eq_sym"];
   note("num_cases",(Forall ("n",
 Or (Atom (R ("=", [Var "n"; Fn ("0", [])])),
  Exists ("m", Atom (R ("=", [Var "n"; Fn ("S", [Var "m"])])))))))
         by ["num_nz"];
   note("suc_inj_eq",(Forall ("m",
 Forall ("n",
  Iff (Atom (R ("=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
   Atom (R ("=", [Var "m"; Var "n"])))))))
     by ["suc_inj"; "suc_cong"];
   so conclude
     (Forall ("m",
 ...
 Forall ("n",
  Imp
   ...
   Imp (Atom (R ("<", [Var "m"; Var "n"])), False)))))
     by ["eq_refl"; "lt_cong"];
   have (Atom (R ("<=", [Fn ("0", []); Fn ("0", [])]))) by ["le_def"; "add_0"];
   so have (Forall ("x",
 Imp (Atom (R ("=", [Var "x"; Fn ("0", [])])),
  Atom (R ("<=", [Var "x"; Fn ("0", [])])))))
 ...
 Or (Atom (R ("<=", [Var "n"; Fn ("0", [])])),
  Exists ("m", Atom (R ("=", [Fn ("S", [Var "m"]); Var "n"]))))))
   ...
   note("add_eq_0",Forall ("m",
 Forall ("n",
  Imp (Atom (R ("=", [Fn ("+", [Var "m"; Var "n"]); Fn ("0", [])])),
   And (Atom (R ("=", [Var "m"; Fn ("0", [])])),
    Atom (R ("=", [Var "n"; Fn ("0", [])]))))))) proof
    [fix "m"; fix "n";
     assume ["A",Atom (R ("=", [Fn ("+", [Var "m"; Var "n"]); Fn ("0", [])]))];
     cases (Or (Atom (R ("=", [Var "m"; Fn ("0", [])])),
 Exists ("p", Atom (R ("=", [Var "m"; Fn ("S", [Var "p"])]))))) by ["num_cases"];
       so conclude (Atom (R ("=", [Var "m"; Fn ("0", [])]))) at once;
       so have (Atom (R ("=",
   [Fn ("+", [Var "m"; Var "n"]); Fn ("+", [Fn ("0", []); Var "n"])]))) by ["add_cong"; "eq_refl"];
       so our thesis by ["A"; "add_0"; "eq_sym"; "eq_trans"];
     qed;
       so consider ("p",Atom (R ("=", [Var "m"; Fn ("S", [Var "p"])]))) at once;
       so have (Atom (R ("=",
   [Fn ("+", [Var "m"; Var "n"]);
    Fn ("+", [Fn ("S", [Var "p"]); Var "n"])]))) by ["add_cong"; "eq_refl"];
       so have (Atom (R ("=",
   [Fn ("+", [Var "m"; Var "n"]);
    Fn ("S", [Fn ("+", [Var "p"; Var "n"])])]))) by ["eq_trans"; "add_suc"];
       so have (Atom (R ("=", [Fn ("S", [Fn ("+", [Var "p"; Var "n"])]); Fn ("0", [])]))) by ["A"; "eq_sym"; "eq_trans"];
       so our thesis by ["not_suc_0"];
     qed];
   so conclude (Forall ("n", Imp (Atom (R ("<=", [Var "n"; Fn ("0", [])])),
  Atom (R ("=", [Var "n"; Fn ("0", [])]))))) by ["le_def"];
   have (Forall ("m",
 Forall ("n",
  Imp (Atom (R ("<=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
   Atom (R ("<=", [Var "m"; Var "n"])))))) proof
    [fix "m"; fix "n";
     assume ["lesuc",(Atom (R ("<=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])))];
     so consider("d",(Atom (R ("=",   [Fn ("+", [Fn ("S", [Var "m"]); Var "d"]); Fn ("S", [Var "n"])])))) by ["le_def"];
     so have (Atom (R ("=",
   [Fn ("S", [Fn ("+", [Var "m"; Var "d"])]); Fn ("S", [Var "n"])]))) by ["add_suc"; "eq_sym"; "eq_trans"];
     so have (Atom (R ("=", [Fn ("+", [Var "m"; Var "d"]); Var "n"]))) by ["suc_inj"];
     so conclude (Atom (R ("<=", [Var "m"; Var "n"]))) by ["le_def"];
     qed];
   so conclude (Forall ("m", Forall ("n",
  Imp (Atom (R ("<=", [Fn ("S", [Var "m"]); Fn ("S", [Var "n"])])),
   Atom (R ("<=", [Var "m"; Var "n"])))))) at once;
   so conclude (Forall ("m", Forall ("n",
  Imp (Atom (R ("<", [Var "m"; Fn ("S", [Var "n"])])),
   Atom (R ("<=", [Var "m"; Var "n"])))))) by ["lt_def"];
   fix "n";
   assume ["hyp",(Atom (R ("<", [Var "n"; Fn ("0", [])])))];
   so have (Atom (R ("<=", [Fn ("S", [Var "n"]); Fn ("0", [])]))) by ["lt_def"];
   so consider("d",(Atom (R ("=", [Fn ("+", [Fn ("S", [Var "n"]); Var "d"]); Fn ("0", [])])))) by ["le_def"];
   so have (Atom (R ("=", [Fn ("S", [Fn ("+", [Var "n"; Var "d"])]); Fn ("0", [])]))) by ["add_suc"; "eq_trans"; "eq_sym"];
   so our thesis by ["not_suc_0"];
   qed];;
