...
*)
  ...
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_BUF")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (path out, SCALAR,
                                            TLIST
                                              [TRIPLE(CELLPIN, ID (enterid "A"), src');
                                               TRIPLE(CELLPIN, ID (enterid "Y"), dst)])])) ()
...
and monadic_flipflop (tbl:tbl) syms path wid (dst,src) = function
	...
	Minimap.declare tbl.namtab syms out (wid-1) 0;
	assert (wid>0);
	Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_DFF_SYNC")), DOUBLE(HASH, TLIST [INT wid]),
                                       TLIST
					 [TRIPLE
                                             (path out, SCALAR,
                                              TLIST
						[TRIPLE(CELLPIN, ID (enterid "D"), src);
						  TRIPLE(CELLPIN, ID (enterid "CK"), clk);
						  TRIPLE(CELLPIN, ID (enterid "Q"), dst)])])) ()
...
and plus exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_ARI_ADD")), DOUBLE(HASH, TLIST [INT wid;INT lwid;INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
                                              [TRIPLE(CELLPIN, ID (enterid "A"), lft);
                                               TRIPLE(CELLPIN, ID (enterid "B"), rght);
                                               TRIPLE(CELLPIN, ID (enterid "Y"), ID out)])])) (); (ID out,wid)
...
and minus exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (lwid>0 && rwid>0);
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_ARI_SUB")), DOUBLE(HASH, TLIST [INT wid;INT lwid;INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and times' exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (lwid>0 && rwid>0);
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_ARI_MUL")), DOUBLE(HASH, TLIST [INT lwid;INT lwid;INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and wildequal exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_EQ")), DOUBLE(HASH, TLIST [INT lwid; INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
                                              [TRIPLE(CELLPIN, ID (enterid "A"), lft);
                                               TRIPLE(CELLPIN, ID (enterid "B"), rght);
                                               TRIPLE(CELLPIN, ID (enterid "Y"), ID out)])])) (); (ID out,1)
...
and sleft' exp (tbl:tbl) syms (lft,lwid) (rght,rwid) = match rght with
  ...
  | _ ->
      ...
      assert (lwid>0 && rwid>0);
      assert (tbl.map_hls.map_id=Map_hls);
      Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid "DVL_SH_L"), DOUBLE(HASH, TLIST [INT wid;INT lwid;INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and sright' exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (lwid>0 && rwid>0);
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_SH_R")), DOUBLE(HASH, TLIST [INT lwid;INT lwid;INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and mux2 exp (tbl:tbl) syms (sel,_) (hi,hwid) (lo,lwid) =
  ...
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_MUX")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
                                              [TRIPLE(CELLPIN, ID (enterid "A"), lo');
                                               TRIPLE(CELLPIN, ID (enterid "B"), hi');
                                               TRIPLE(CELLPIN, ID (enterid "S"), sel);
                                               TRIPLE(CELLPIN, ID (enterid "Y"), out')])])) (); (out',wid)

and redand exp (tbl:tbl) syms (src,wid) =
  let out = Minimap.netid "rand" tbl.gentab tbl.namtab in
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_RED_AND")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and redor exp (tbl:tbl) syms (src,wid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_RED_OR")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and pling exp (tbl:tbl) syms (src,wid) =
  ...
  assert (wid>0);
  let src' = shorten syms wid src in
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_LOG_NOT")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and tilde exp (tbl:tbl) syms (src,wid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_BW_NOT")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and logand exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_BW_AND")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and logor exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_BW_OR")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and logxor exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_BW_XOR")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and log_pand exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_LOG_AND")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and log_por exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_LOG_OR")), DOUBLE(HASH, TLIST [INT wid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and noteq exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_NEQ")), DOUBLE(HASH, TLIST [INT lwid; INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and gte exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_GEQ")), DOUBLE(HASH, TLIST [INT lwid; INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and greater exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_GREATER")), DOUBLE(HASH, TLIST [INT lwid; INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and less exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_LESS")), DOUBLE(HASH, TLIST [INT lwid; INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and lte exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (enterid ("DVL_LEQ")), DOUBLE(HASH, TLIST [INT lwid; INT rwid]),
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
| DOUBLE(ALWAYS,stmts) ->
  ...
  | TLIST [DOUBLE(DOUBLE(AT, TLIST [DOUBLE ((POSEDGE|NEGEDGE) as edg, clk);
           ...
           QUADRUPLE
             (IF, clr1,
              QUADRUPLE ((ASSIGNMENT|DLYASSIGNMENT) as dly, dest, _, clrval),
              TRIPLE
                (IF, clken, (QUADRUPLE ((ASSIGNMENT|DLYASSIGNMENT) as dly1, dest1, _, _) as itm))))] ->
    if !verbose then printf "pattern matched %s\n" (Dump.dumpstr stmts);
    if clr=clr1 && dly=dly1 && dest=dest1 then edge hashtab syms2 (tbl:tbl) [DOUBLE(edg,clk);TRIPLE(edg1,clr,clrval);DOUBLE(dly,clken)] itm
  ...
  | TLIST [DOUBLE(DOUBLE(AT, TLIST [DOUBLE ((POSEDGE|NEGEDGE) as edg, clk);
       ...
       QUADRUPLE
        (IF, clr1,
         QUADRUPLE ((ASSIGNMENT|DLYASSIGNMENT) as dly, dest, _, clrval),
         (QUADRUPLE ((ASSIGNMENT|DLYASSIGNMENT) as dly1, dest1, _, _) as itm)))] ->
    if !verbose then printf "pattern matched %s\n" (Dump.dumpstr stmts);
    if clr=clr1 && dly=dly1 && dest=dest1 then edge hashtab syms2 (tbl:tbl) [DOUBLE(edg,clk);TRIPLE(edg1,clr,clrval)] itm	   
    else failwith (sprintf "malformed always block %s != %s or %s != %s or %s != %s"
                     (Count.tokenstr clr)
                     (Count.tokenstr clr1)
                     (Count.tokenstr dly)
  ...
  | TLIST [DOUBLE(DOUBLE(AT, TLIST [DOUBLE ((POSEDGE|NEGEDGE) as edg, clk);
                             DOUBLE ((POSEDGE|NEGEDGE) as edg1, clr);
                             DOUBLE ((POSEDGE|NEGEDGE) as edg2, pre)]), itm)] ->
    if !verbose then printf "pattern matched %s\n" (Dump.dumpstr stmts);
    edge hashtab syms2 (tbl:tbl) [DOUBLE(edg,clk);DOUBLE(edg1,clr);DOUBLE(edg2,pre)] itm	   
    ...
    mybuf.mux.nam.id <> "" then begin
      match itm.tree with
	    ...
	    let modul = synth_module select itm.symbols synthnam params arg3 arg4 arg5 in
            ...
            then failwith("generate_submod select "^arch^" "^nam^" result is ambiguous");
