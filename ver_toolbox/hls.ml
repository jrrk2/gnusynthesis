...
if not (Hashtbl.mem dvlhash pth) then (Hashtbl.add dvlhash pth (); match List.hd lst with
  | "DVL_ARI_ADD" -> ()
  | "DVL_ARI_MUL" -> ()
  | "DVL_ARI_SUB" -> ()
  | "DVL_BUF" -> ()
  ...
  | "DVL_SH_L" -> ()
  | "DVL_SH_R" -> ()
  | oth -> failwith ("Unmapped library cell: "^oth));
enterid pth
...
*)
  ...
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_BUF" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (path out, SCALAR,
                                            TLIST
...
and monadic_flipflop (tbl:tbl) syms path wid (dst,src) = function
	...
	Minimap.declare tbl.namtab syms out (wid-1) 0;
	assert (wid>0);
	Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_DFF_SYNC" :: string_of_int wid :: [])), EMPTY,
                                       TLIST
					 [TRIPLE
                                             (path out, SCALAR,
                                              TLIST
...
and plus exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_ARI_ADD" :: string_of_int wid :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and minus exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (lwid>0 && rwid>0);
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_ARI_SUB" :: string_of_int wid :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and times' exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (lwid>0 && rwid>0);
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_ARI_MUL" :: string_of_int lwid :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and wildequal exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_EQ" :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and sleft' exp (tbl:tbl) syms (lft,lwid) (rght,rwid) = match rght with
  ...
  | _ ->
      ...
      assert (lwid>0 && rwid>0);
      assert (tbl.map_hls.map_id=Map_hls);
      Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_SH_L" :: string_of_int wid :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and sright' exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  assert (lwid>0 && rwid>0);
  assert (tbl.map_hls.map_id=Map_hls);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_SH_R" :: string_of_int lwid :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and mux2 exp (tbl:tbl) syms (sel,_) (hi,hwid) (lo,lwid) =
  ...
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_MUX" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and redand exp (tbl:tbl) syms (src,wid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_RED_AND" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and redor exp (tbl:tbl) syms (src,wid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_RED_OR" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and pling exp (tbl:tbl) syms (src,wid) =
  ...
  assert (wid>0);
  let src' = shorten syms wid src in
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_LOG_NOT" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and tilde exp (tbl:tbl) syms (src,wid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_BW_NOT" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and logand exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_BW_AND" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and logor exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_BW_OR" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and logxor exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_BW_XOR" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and log_pand exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_LOG_AND" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and log_por exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out (wid-1) 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_LOG_OR" :: string_of_int wid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and noteq exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_NEQ" :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and gte exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_GEQ" :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and greater exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_GREATER" :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and less exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_LESS" :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
...
and lte exp (tbl:tbl) syms (lft,lwid) (rght,rwid) =
  ...
  Minimap.declare tbl.namtab syms out 0 0;
  assert (wid>0);
  Hashtbl.add tbl.snd (QUADRUPLE(MODINST, ID (cacheid ("DVL_LEQ" :: string_of_int lwid :: string_of_int rwid :: [])), EMPTY,
                                     TLIST
                                       [TRIPLE
                                           (instid' tbl, SCALAR,
                                            TLIST
